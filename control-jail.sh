...
# return: 1,running; 0, not running;
is_in_service() {
    http_code=$(curl -I -m 10 -o /dev/null -s -w %{http_code} http://${HTTP_HOST}:${HTTP_PORT}/actuator/linux/check)
...
start() {
    is_running
    if  [ 1 == $? ]; then
        echo "${DESC} is already running. You can visit http:/${HTTP_HOST}:${HTTP_PORT} on this host to manage the wireless network."
        exit
    fi
    ...
    LD_PRELOAD=${LD_PRELOAD} ${JSVC} ${JSVC_OPTS} ${MAIN_CLASS} start
    ...
    if  [ 1 == $? ] || [ 2 == $? ]; then
        ...
        echo You can visit http://${HTTP_HOST}:${HTTP_PORT} on this host to manage the wireless network.
...
status() {
    ...
    if  [ 0 == $? ]; then
        ...
        echo "${DESC} is running. You can visit http://${HTTP_HOST}:${HTTP_PORT} on this host to manage the wireless network."
    fi
}
